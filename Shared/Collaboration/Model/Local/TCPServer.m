...
/*
- (BOOL)acceptConnectionFromPeer:(NSData *)peer {
	if (self.delegate && [self.delegate respondsToSelector:@selector(didAcceptConnectionForPeer:)]) { 
        [self.delegate didAcceptConnectionForPeer:peer];
    }
...
static void TCPServerAcceptCallBack(CFSocketRef socket, CFSocketCallBackType type, CFDataRef address, const void *data, void *info) {
    ...
    if (kCFSocketAcceptCallBack == type) { 
		...
		//if ([server acceptConnectionFromPeer:peer]) {
			//DLog(@"peer=%@, %@", peer, [NSString stringWithUTF8String:[peer bytes]]);
...
- (BOOL)isStopped {
	return NULL == _ipv4socket && !self.netService;
}
...
{
	// This was causing a crash on resume:
	// 'NSObject' may not respond to '-netServiceDidPublish:'
